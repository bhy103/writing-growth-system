...
const vocabularySchema = {
  ...
  properties: {
    ...
    vocabularySection: {
      type: "string",
      description:
        "Part 1. Vocabulary study notes. Use one entry per source word: word - easy-sound-chunks /IPA/, then one or more part-of-speech definition blocks with exactly two example sentences each.",
    },
    worksheet: {
      type: "string",
      description:
...
function buildFallbackPack(words: string[], title: string): VocabularyStudyPack {
  const vocabularySection = words
    .map((word, index) =>
      [
        `${index + 1}. ${word} - ${word.toLowerCase()} /(pronunciation)/`,
        "(n.) Add a simple meaning here after AI is configured.",
...
function buildPrompt({ gradeLevel, title, words }: { gradeLevel?: string | null; title: string; words: string[] }) {
  return [
    ...
    "Create exactly one vocabulary entry/header for each source word.",
    "Do not create a second header for the same word, even if the word has noun and verb meanings.",
    "Format each source word like this exact pattern:",
    "word - easy-sound-chunks /IPA/",
    "",
    "Then place every meaning for that same word underneath that one header:",
    "(pos.) simple meaning",
    "eg. First student-friendly example sentence.",
    "    Second student-friendly example sentence.",
    "",
    "If the same word has multiple meanings or parts of speech, keep them under the same word header like this:",
    "document - do-cu-ment /pronunciation/",
    "(n.) a written or printed paper",
    "eg. Please read the document carefully.",
    "    He signed the document.",
    "(v.) to record information",
    "eg. They documented the results.",
    "    She documented every step.",
    "",
    "Do not use labels such as 'Meaning 1' or 'mem:'.",
    "Do not number meanings with 'Meaning 1'.",
    "Do not repeat the word header for a second meaning.",
    "",
    "EASY SOUND CHUNK RULES",
    "For every word, include easy-sound-chunks immediately after the dash.",
    "The chunks are for children to read and remember the sound of the word, not strict dictionary syllables.",
    "Use lowercase letters and hyphens.",
    "Prefer clear pronounceable chunks that match how the word is commonly said.",
    "Examples:",
    "phenomenon - phe-no-me-non /pronunciation/",
    "avalanche - a-va-lanche /pronunciation/",
    "hemisphere - he-mi-sphere /pronunciation/",
    "precipitation - pre-ci-pi-ta-tion /pronunciation/",
    "flurry - flur-ry /pronunciation/",
    "",
    "WORKSHEET RULES",
