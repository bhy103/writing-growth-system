...
const vocabularySchema = {
  ...
  properties: {
    ...
    vocabularySection: {
      type: "string",
      description:
        "Part 1. Vocabulary study notes. Use the format: word - easy-sound-chunks /IPA/ (pos.), then a simple definition line and exactly two example sentences for each meaning.",
    },
    worksheet: {
      type: "string",
      description:
...
function buildFallbackPack(words: string[], title: string): VocabularyStudyPack {
  const vocabularySection = words
    .map((word, index) =>
      [
        `${index + 1}. ${word} - ${word.toLowerCase()} /(pronunciation)/ (n.)`,
...
function buildPrompt({ gradeLevel, title, words }: { gradeLevel?: string | null; title: string; words: string[] }) {
  return [
    ...
    "Format each word like this exact pattern:",
    "word - easy-sound-chunks /IPA/ (pos.)",
    "(pos.) simple meaning",
    ...
    "Do not number meanings with 'Meaning 1'. If a word has multiple meanings, repeat the part-of-speech definition line and two examples under the same word.",
    "",
    ...
    "Examples:",
    "phenomenon - phe-no-me-non /fəˈnɒmɪnən/ (n.)",
    "avalanche - a-va-lanche /ˈævəlɑːntʃ/ (n.)",
    "hemisphere - he-mi-sphere /ˈhemɪsfɪə/ (n.)",
    "precipitation - pre-ci-pi-ta-tion /prɪˌsɪpɪˈteɪʃən/ (n.)",
    "flurry - flur-ry /ˈflʌri/ (n.)",
    "",
    "WORKSHEET RULES",
